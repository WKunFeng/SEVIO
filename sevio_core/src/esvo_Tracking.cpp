  ...
  ets_(IDLE)
{
  ...
  // trans from left event camera to imu
  ...
  //receive imu_pose from mapping thread
  imuPose_sub_ = nh_.subscribe("imu_pose", 0, &esvo_Tracking::imuPoseCallback, this);
...
//  LOG(INFO) << "SYSTEM STATUS(T"
  if(ESVO_System_Status_ == "INITIALIZATION" && ets_ == IDLE)
  {
    ...
    Eigen::Matrix4d T_w_lcam = imu_pose_*T_imu_lcam;
    Eigen::Quaterniond quaternion;
    quaternion=T_w_lcam.block<3,3>(0,0);
    T_w_lcam.block<3,3>(0,0) = quaternion.normalized().toRotationMatrix();
    //LOG(INFO) << T_w_lcam(0,3) << T_w_lcam(1,3) << T_w_lcam(2,3);
    ref_.tr_ = Transformation(imu_pose_);
  }
...
void esvo_Tracking::imuPoseCallback(const geometry_msgs::PoseStampedConstPtr &msg)
{
  std::lock_guard<std::mutex> lock(data_mutex_);
  //
  Eigen::Quaterniond q_imu_pose = Eigen::Quaterniond(
          msg->pose.orientation.w,
          msg->pose.orientation.x,
          msg->pose.orientation.y,
          msg->pose.orientation.z);
  Eigen::Vector3d t_imu_pose = Eigen::Vector3d(
          msg->pose.position.x,
          msg->pose.position.y,
          msg->pose.position.z);
  //imu_pose_ = Eigen::Matrix4d::Identity();
  imu_pose_.block<3,3>(0,0) = q_imu_pose.normalized().toRotationMatrix();
  imu_pose_.block<3,1>(0,3) = t_imu_pose;
}
